...
def fig_consensus_effort(csv_dir: Path, fig_dir: Path) -> None:
    ...
    ax0.set_ylabel("LLM calls per instance")
...
def fig_pareto(csv_dir: Path, fig_dir: Path) -> None:
    ...
    ax.set_xlabel("Per-instance LLM-call budget (cap)")
...
def fig_thought_action_phase(csv_dir: Path, fig_dir: Path) -> None:
    """Per-phase mean THOUGHT-block / action character counts (resolved only).

    Two panels:
      (a) Side-by-side bars per phase: THOUGHT-block vs action chars per
          assistant message.
      (b) THOUGHT-block:action ratio per phase, resolved vs unresolved.
    """
    ...
    ax_a.bar(xs - width / 2, res["mean_thought_chars_per_msg"], width=width,
             color=RESOLVED_COLOR, label="THOUGHT-block chars / msg",
    ...
    ax_a.set_title("(a) THOUGHT text vs. action by phase")
    ax_a.legend(loc="upper right", fontsize=6.5)
    ...
    ax_b.set_xticklabels([p.replace("_", "\n") for p in PHASES_ORDERED],
                         fontsize=6.5)
    ax_b.set_ylabel("THOUGHT-block : action chars")
    ax_b.set_title("(b) Observable text-action balance")

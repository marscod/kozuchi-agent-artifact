...
27 B agent unchanged on Multi-SWE-bench Java Verified resolves 41/128
instances (32.03\%), placing first among strict open-weight submissions
and fourth of 42 overall. Per-phase scaffold behaviour matches the
Python run within $\pm 5$ percentage points on every phase, evidence that
the harness is language agnostic. It ranks 12th among 135 public Python
submissions and first among open-weight systems. The analysis shows that
...
advantages, while the CI workflow compresses operator-facing touch-points
from five to one.
...
industrial demand: developers want an LLM-based agent that takes an
issue description plus a repository state and emits a correct,
test-passing patch. In SWE-bench-style
deployments~\cite{jimenez2024swebench}, meeting that demand requires
...
Four operational blockers shaped the design.
...
\begin{figure}[t]
  ...
  SWE-bench Verified and Multi-SWE-bench Java Verified.}
...
to address those four pains, and on what we learned operating it.
...
and guarded editing. Together these choices turn a free-form repair loop
into an inspectable workflow whose state can be resumed and compared
across open-weight and API-served backends.
...
% source: experiments/evaluation/verified/20260326_kozuchi-mini-swe-agent_qwen3.5-27b/src/{*.py, build.sh}
...
across heterogeneous clusters. Its reuse mode allows operators to
short-circuit expensive stages using previously validated outputs, while
...
THOUGHT-block-to-action balance, rollback language, and error-marker
asymmetries.
...
through the SWE-bench harness. On Multi-SWE-bench Java Verified, the
...
\textbf{Roadmap.}
\S\ref{sec:related} situates the work among agentic SWE systems,
industry-facing coding agents, training pipelines, and LLM-enabled
CI/CD systems. \S\ref{sec:background} sets the SWE-bench, cluster,
and prior-workflow background. \S\ref{sec:definitions} defines the
runtime, trajectory, phase graph, tool-call contract,
verifier/selector, and artifact boundary, and lists five invariants
the runtime maintains.
\S\ref{sec:observations} narrates five empirical observations that
shaped the design. \S\ref{sec:design} describes the system in five
layers (runtime, tools, pipeline, serving, CI).
\S\ref{sec:design-tts} details the cross-agent test-time selector.
\S\ref{sec:eval} answers six research questions on SWE-bench
Verified and Multi-SWE-bench Java Verified, all of them grounded in
scripted analyses over the published Python TTS@8 and Java xcheck@8 artifacts: headline accuracy
with uncertainty, peer-ranked position with multiplicity-corrected paired
tests, structural breakdowns by repository, patch size, and trajectory
effort, the failure taxonomy and operational-reliability profile, the
TTS@8 selection lift and cost--diversity envelope, and cross-language
transfer to Java using the same agent, backbone, and scaffold.
\S\ref{sec:artifact} gives the artifact availability statement and
release boundary for the public trajectories, reports, selector bundle,
and analysis scripts.
\S\ref{sec:lessons} states seven operational lessons.
\S\ref{sec:conclusion} discusses future directions.
...
in RQ1--RQ5.  Multi-SWE-bench Java Verified is the corresponding Java
...
Multi-SWE-bench Java Verified leaderboard, keeping each benchmark's
...
Modern SWE agents follow an agentic loop: a tool-using LLM observes a
shell, plans, runs a command, observes the result, and repeats~\cite{yao2023react,shinn2023reflexion}. We build
on a minimal open-source runtime~\cite{miniswea} with harness-level
support for long repairs: phase control, persistent shared state,
deterministic tools, and model-independent action formatting. The
artifact appendix defines the release boundary: trajectories, selector
choices, evaluation outputs, and deep analysis are public, while agent
and pipeline source code are not.

\subsection{Multi-cluster operating context}
% source: scripts/module_load.sh (hostname-prefix branching);
%         paper/final/stats/pipeline_inventory.json
Our infrastructure is heterogeneous and shared across many groups.
Candidate generation, grading, and optional training run on different
GPU and VM-backed clusters, each with different module systems,
container support, storage paths, and queue behavior. The central
methodological point is that the pipeline hides those differences
behind a single cluster abstraction: a stage describes what work it
needs, while the environment layer supplies the local launch details.
Concrete cluster names and runner wiring are listed with the released
artifact rather than in the main narrative.

\subsection{The workflow we set out to replace}
% source: README.md (Use), docs/HOW_TO_DEVELOP.md, pipeline/* scripts.
Before Kozuchi Agent, each iteration required manually starting a vLLM
server, launching a one-off repair-benchmark run, waiting hours, running Docker
grading, and recording scores in a spreadsheet. SFT and RL experiments
used separate branches and scripts; changing models or comparing
trajectories required file edits and manual log reading, which are error prone. The harness
replaces this with a CI-driven pipeline, one configurable agent, a fixed
runtime contract, and a publishable artifact bundle. Table~\ref{tab:workflow-replacement}
summarizes the workflow replacement; the engineer-minute estimate is
bounded by published SWE-bench Docker-runtime and CI-adoption
studies~\cite{epoch2024swebenchdocker,hilton2016ci}.
...
The harness design emerged from operational failures in early
prototypes; each observation below maps to a mechanism in
\S\ref{sec:design}.

\paragraph{O1 (Long conversations forget).}
% source: configs/agent_sota.yaml agent.phases + system_template
Single-prompt repair attempts skipped reproduction, re-investigated old
locations, or patched code they had not understood. We responded with an
explicit phase sequence and required the agent to state its workflow
position at every turn.

\paragraph{O2 (Tool syntax is a cliff).}
% source: configs/agent_sota.yaml agent.action_format + action_format_inventory.json
Different model families emit different tool-call bracketings. Separating
the logical action from its surface syntax made model switching a
configuration change rather than an agent fork.

\paragraph{O3 (State must survive compression).}
% source: configs/agent_sota.yaml instance_template + phases.*.definition
Evidence stored only in conversation text can disappear when context is
compressed. Reproduction notes, generated tests, traces, task boards, and
handover memos therefore live in shared filesystem state.

\paragraph{O4 (Shell alone localizes poorly).}
% source: swe_sota_agent/tools/{line_trace,caller_trace,line_edit}.py + configs/agent_sota.yaml agent.tools
Root-cause localization benefits from classic dynamic analysis. We added
a small deterministic tool sandbox for line tracing, caller discovery,
and guarded editing rather than a large, brittle tool zoo.

\paragraph{O5 (CI cost is repeated work).}
% source: README.md CI reuse mode + .gitlab-ci.yml reuse variables
Benchmark stages take hours. Reuse mode lets operators hold validated
artifacts fixed while changing one stage, which made repeated TTS@8 and
selector sweeps practical.
...
cross-agent selection converts those independently chosen tests into the
$B_{ij}$ and $R_{ij}$ signals used below.
...
% source: README.md (CI reuse mode) + .gitlab-ci.yml variables block
Reuse mode lets the team hold a trajectory bundle fixed while
experimenting with selector weights, or hold an environment fixed while
testing a new model. It is the operational difference between a
benchmark script and an industrial evaluation pipeline.
...
$\arg\max_i \mathrm{score}(i)$, breaking ties by patch length. A tie
...
different parts of SWE-bench Verified. The candidate-level decomposition
sharpens the bound: for 92 instances, none of the eight runs produces a
...
uses Multi-SWE-bench Java Verified as a cross-language validation track.
...
the model to the agent runtime. For Python we run the same model eight times,
...
uses the published Multi-SWE-bench Java Verified submission bundle with
...
position analysis to Java, where the same 27 B configuration ranks first
...
\caption{Artifact-based TTS@8 ablation.}
...
selection pattern on Multi-SWE-bench Java Verified.  Only the corpus and
...
with Wilson intervals~\cite{wilson1927score} and per-phase assistant-message share.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Production Planning and Lessons Learned
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Production Planning and Lessons Learned}
\label{sec:lessons}
...
The deployment lessons are operational rather than architectural.  First,
phase boundaries beat free-form planning at long horizons: every Python
...
lessons: the workflow compresses five operator touch-points to one CI
push (about 70 engineer-minutes saved per cycle), the runtime exposes
four action formats across 17 model configs, six of nine CI stages are
reusable, and five cluster environments sit behind \code{ENV\_NAME}.  On
Python, 494/495 produced patches apply cleanly and the cloud-vs-Docker
score drift is only 374 vs. 376 out of 500.  On Java, the same runtime
contract reaches 41/128 (32.03\%) and ranks first among strict
open-weight submissions, making cross-language transfer a measured
operational outcome rather than an anecdotal consistency check.
...
Kozuchi Agent targets conservative production use on large repositories,
with evidence from SWE-bench Verified (Python) and Multi-SWE-bench Java
Verified. Using a locally hosted open-weight model, no fine-tuning,
...
Multi-SWE-bench Java Verified instances (32.03\%) under the same agent,
...
while ablations isolate only candidate count and selector signals. The
phase decomposition, action grammar, persistent state, and cross-agent
selector transfer from Python to Java unchanged, supporting the
language-agnostic claim. The selector adds +14 resolved Python instances;
...
Industrial-impact claims remain limited to workflow replacement,
cross-evaluator consistency, and the Python--Java benchmark evidence;
Java cross-track results are first-class evidence rather than a single
consistency point, while controlled pilots and broader multilingual
...
The source index for the references below is maintained in \code{paper/final/source-index.md};
it records the URL and paper-specific use of each external source.

The public artifact is the SWE-bench Verified submission folder
at \url{https://github.com/kimusaku/experiments}. It contains the 495
trajectories (\code{trajs/}), harness reports (\code{logs/}),
predictions (\code{all\_preds.jsonl}), cross-agent selection bundle
(\code{xcheck/}), and released analysis scripts under \code{src/}. From
these files alone, \code{src/build.sh} regenerates the empirical
RQ1--RQ5 values; the Java submission directory under the same artifact
family regenerates the RQ6 values. All RQ figures,
trajectory/conversation audits, and the selector re-run over the eight
published candidate streams are included in that artifact family.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Hidden Data Availability Statement (not rendered in this review build)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\iffalse
\section{Data Availability Statement}
\label{sec:data}

% Mandatory per ASE 2026 CFP (paper/ase2026/cfp.md): placed after Conclusion
% and within the page limit; describe artifacts and DOI.
The public artifact backing this paper consists of the SWE-bench
Verified submission materials and trajectory bundle described in
\S\ref{sec:eval}: predictions, results, metadata, eight per-run
trajectory sets, cross-agent test tables, selector outputs, CSVs,
figures, and deep-analysis summaries. All artifacts and additional deep
analysis can be found there. Cluster-specific
paths (e.g., \code{/home/share-tmp/frj/takane-for-coding},
\code{/lustre/share/takane-for-coding}) are internal and not part of
the public artifact.
Model weights for \code{Qwen/Qwen3.5-27B} and the other backends
inventoried in \code{\statpath{model\_inventory.tex}}
(Table~\ref{tab:models}) must be obtained from their respective
providers under their licenses. Some cluster-specific runner
configuration (GitLab runner tokens, internal SLURM partitions) is
omitted intentionally.

\begin{table}[t]
\centering
\scriptsize
\caption{Backend serving inventory.}
\label{tab:models}
\input{\statpath{model_inventory.tex}}
\end{table}
\fi
...
THOUGHT-block-to-action ratio (Cliff's $\delta{=}+0.13$). These

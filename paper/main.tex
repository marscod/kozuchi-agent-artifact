...
%% Camera-ready class required by the track:
...
\acmDOI{10.1145/3832783.3834531}
\acmISBN{979-8-4007-2882-2/2026/10}
\acmSubmissionID{ase26ind-p408-p}
\received{2026-05-01}
\received[accepted]{2026-07-01}
...
%% Prevent single-line paragraph fragments at column/page boundaries.
\clubpenalty=10000
\widowpenalty=10000
...
\title[Kozuchi Agent: A Language-Agnostic Open-Weight Agent\ldots]{Kozuchi Agent: A Language-Agnostic Open-Weight Agent for Software Repair}
...
%% Each author is declared with its own \author command so that the ACM
%% camera-ready validator counts all 27 authors, but a single shared
%% \affiliation keeps them in one title box. acmart stacks the names of a
%% box one per line; with 27 authors that fills a page, so we make the
%% separator inline and let the roster wrap across the full text width.
%% This reproduces the original single-block roster with superscript-keyed
%% affiliations.
\settopmatter{authorsperrow=1}
\makeatletter
\newcount\kz@authorcount
\def\@typeset@author@bx{\bgroup\hsize=\author@bx@wd
  \normalbaselines
  \global\setbox\author@bx=\vtop{\centering
    \@authorfont
    %% acmart separates the names of an author box with \par. Turn that
    %% separator into a comma so the 27-name roster wraps as running text
    %% instead of occupying 27 lines.
    \begingroup
      \kz@authorcount=0\relax
      \def\par{\ifnum\kz@authorcount>0\relax,\space\fi
        \global\advance\kz@authorcount by 1\relax}%
      \@currentauthors
    \endgroup
    \endgraf\@affiliationfont\@currentaffiliation}\egroup
  \box\author@bx\hspace{\author@bx@sep}%
  \gdef\@currentauthors{}%
  \gdef\@currentaffiliation{}}
\makeatother

\author{Mehdi Bahrami\textsuperscript{1}}
\author{Kosaku Kimura\textsuperscript{2}}
\author{Satoshi Munakata\textsuperscript{2}}
\author{Satoshi Nakashima\textsuperscript{2}}
\author{Yu Ishikawa\textsuperscript{2}}
\author{Kosuke Maeda\textsuperscript{2}}
\author{Nao Soma\textsuperscript{2}}
\author{Kenichi Kobayashi\textsuperscript{2}}
\author{Keisuke Miyazaki\textsuperscript{2}}
\author{Keizo Kato\textsuperscript{2}}
\author{Shigeki Fukuta\textsuperscript{2}}
\author{Tatsuo Kumano\textsuperscript{2}}
\author{Nobutaka Imamura\textsuperscript{2}}
\author{Kevin Takeshi Musgrave\textsuperscript{1}}
\author{Shahbaz Abdul Khader\textsuperscript{3}}
\author{Kwun Ho Ngan\textsuperscript{3}}
\author{Joseph Townsend\textsuperscript{3}}
\author{Fayas Asharindavida\textsuperscript{3}}
\author{Matthieu Parizy\textsuperscript{3}}
\author{Akira Sakai\textsuperscript{3}}
\author{Yuma Ichikawa\textsuperscript{2}}
\author{Yang Zhao\textsuperscript{4}}
\author{Michiaki Takizawa\textsuperscript{2}}
\author{Taku Fukui\textsuperscript{2}}
\author{Hiroki Ohtsuji\textsuperscript{2}}
\author{Wei-Peng Chen\textsuperscript{1}}
\author{Hiro Kobashi\textsuperscript{2}}
...
%% \shortauthors drives the running page header only.
%% The ACM Reference Format must carry the full roster exactly as the
%% camera-ready validator expects it, so \@acmRefAuthors spells out all
%% 27 co-authors; the single \author{} block above carries affiliation
%% superscripts that must not leak into the reference string. acmart
%% appends a literal ". " after this name, so the trailing period is
%% deliberately omitted here.
\renewcommand{\shortauthors}{Bahrami et al.}
\makeatletter
\newcommand{\@acmRefShortAuthors}{Mehdi Bahrami, Kosaku Kimura, Satoshi
Munakata, Satoshi Nakashima, Yu Ishikawa, Kosuke Maeda, Nao Soma,
Kenichi Kobayashi, Keisuke Miyazaki, Keizo Kato, Shigeki Fukuta, Tatsuo
Kumano, Nobutaka Imamura, Kevin Takeshi Musgrave, Shahbaz Abdul Khader,
Kwun Ho Ngan, Joseph Townsend, Fayas Asharindavida, Matthieu Parizy,
Akira Sakai, Yuma Ichikawa, Yang Zhao, Michiaki Takizawa, Taku Fukui,
Hiroki Ohtsuji, Wei-Peng Chen, and Hiro Kobashi}
\AtBeginDocument{%
...
%% CCS concepts and keywords enabled for the camera-ready build.
\begin{CCSXML}
<ccs2012>
   <concept>
...
\keywords{AI agent, LLM, automated program repair, SWE-bench, agent harness,
...
Industrial software-engineering teams increasingly need LLM agents that
turn bug reports into correct patches, yet benchmark-scale operation adds
long horizons, tool-use discipline, context persistence, heterogeneous
clusters, and evaluation reuse. We present Kozuchi Agent, a
\textit{language-agnostic open-weight repair agent} and \textit{CI-operated
evaluation pipeline}. Explicit phases, persistent state, deterministic
tools, a model-independent action interface, and cross-agent test-time
selection make runs auditable and repeatable. With locally hosted
Qwen3.5-27B, no fine-tuning, and TTS@8, Kozuchi resolves 374/500
SWE-bench Verified instances on the official evaluator. Unchanged on
Multi-SWE-bench Java, the same 27-billion-parameter agent resolves 41/128
instances (\SI{32.03}{\percent}), ranking first among strict open-weight
submissions and fourth of 42 overall; on Python it ranks 12th of 135 and
first among open-weight systems. Per-phase behavior remains within
$\pm 5$ percentage points across languages. Remaining failures mainly
reflect semantic correctness, Java-specific harness issues, and selection
errors. Across both tracks, results compare favorably with open/local
peers by parameter count. Analysis of candidate diversity, selector
regret, and patch reliability shows that the remaining gap is primarily
semantic correctness and selection rather than edit formatting or
...
Operationally, reusable CI stages reduce operator touch-points from five
to one across heterogeneous internal clusters.
...
%% Fig.~1 centered on page~1 in the ACM teaser slot (acmart sets
%% \@topnum=0, so ordinary [t] floats cannot reach page~1). Abstract
%% prints normally below the teaser in the left column.
\begin{teaserfigure}
  \centering
  % source: figures/broad_success_vs_params_with_java.png
  %         (regenerated by paper/comparison/src/plot_broad_with_java.py).
  \includegraphics[width=0.64\textwidth]{figures/broad_success_vs_params_with_java.png}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.9\textwidth]{\figpath{overall-architecture.pdf}}
...
component-removal reruns (Table~\ref{tab:evidence-map}).
...
(\S\ref{sec:conclusion}). Table~\ref{tab:evidence-map} summarizes this
evidence boundary for all mechanisms and claims.
...
On the official SWE-bench Verified cloud evaluator, our 27-billion-parameter open-weight
Python configuration resolves 374 of 500 instances (\SI{74.80}{\percent}, Wilson \SI{95}{\percent} CI [\SI{70.82}{\percent},
\SI{78.41}{\percent}]). The internal Docker re-grade resolves 376 (\SI{75.2}{\percent}); the
...
per-instance McNemar tests. Of 126 unresolved instances, \SI{91.3}{\percent} are
patches that apply cleanly but
do not flip the hidden tests; \SI{0}{\percent} are malformed-diff or empty-patch
...
unchanged 27-billion-parameter Java configuration resolves 41/128 instances (\SI{32.03}{\percent}), ranks
...
medians sit within \SI{20}{\percent} of Python parity, and the same 27-billion-parameter Qwen3.5
...
Table~\ref{tab:evidence-map} separates what this experience report
establishes through controlled comparisons from what it supports only
through operational signatures. ``Ablated'' rows have a matched
comparison in the published artifact; ``operational'' rows are
evidenced by trajectory audits and inventories without
component-removal reruns; ``estimate'' and ``out of scope'' rows are
disclosed as such wherever they are cited.
...
\begin{table}[t]
\centering
% Evidence-boundary summary added for camera-ready in response to
% reviewer requests to separate causal from observational claims.
% Numbers cross-referenced from paper/final/stats/{per_run_pass1.json,
% selector_ablation.json,run_overlap.json,workflow_replacement.json}.
\caption{Evidence map: mechanisms and claims by evidence type.}
\label{tab:evidence-map}
\begin{tabular}{@{}p{0.4\columnwidth}p{0.36\columnwidth}l@{}}
\toprule
Mechanism / claim & Evidence type & Where \\
\midrule
Candidate count $K{=}8$ & Ablated: pass@1 334--343 vs.\ oracle 408 & Tab.~\ref{tab:selector-ablation} \\
Cross-agent selector & Ablated: 376 vs.\ 362 (order) vs.\ 346 (self-tests) & Tab.~\ref{tab:selector-ablation} \\
Selector weights $w_B,w_R$ & Sensitivity sweep: $\le$5/500 outcome change & \S\ref{sec:design-tts} \\
Phase graph, handover state, formatter, tool sandbox, CI reuse & Operational signatures; not ablated & \S\ref{sec:design}, \S\ref{sec:lessons} \\
Engineer-minute saving & Derived order-of-magnitude estimate & Tab.~\ref{tab:workflow-replacement} \\
GPU-hours & Wall-clock-derived estimate & \S\ref{subsec:rq5} \\
Production deployment, developer pilot, GPU accounting & Out of scope; future work & \S\ref{sec:conclusion} \\
\bottomrule
\end{tabular}
\end{table}
...
In sum, the individual harness mechanisms---phased workflows, persistent
state, runtime tools, CI orchestration---draw on established practice.
The originality of this work lies in the cross-agent selector
formulation, in which independent runs generate tests that are
cross-applied as selection evidence under a constraint that no hidden or
production-only feedback is available at selection time, and in the
integration and industrial-scale evaluation of these mechanisms as one
auditable harness.
...
structure does not make the agent more capable by itself; rather, it is
designed to keep long trajectories from becoming unstructured
conversations.
...
fixed global constants in the submitted selector artifact; they are not
learned and were not tuned on benchmark outcomes. The asymmetry encodes
a deliberate engineering prior for a merge-gated setting: a patch that
silently breaks existing behaviour (a regression that a
\code{PASS\_TO\_PASS} suite would catch) is the costlier failure once
merged, whereas agent-generated bug-revealing tests are sparser and
noisier as evidence, so regression conformance is weighted higher than
bug-revealing pass rate. Nevertheless, an artifact-only sweep over seven
nearby weight pairs changes at most 35/495 selected candidates and
5/500 estimated outcomes ($\le$\SI{1.0}{\percentagepoint}), so we report the submitted pair
as a robust operating point rather than an optimized one; a systematic
weight search on held-out tasks remains future work. We then pick
...
$376/408=\SI{92.2}{\percent}$ of the oracle union ceiling under the internal Docker
...
34 instances (\SI{6.8}{\percentagepoint}) below the oracle union; linear interpolation on
...
\midrule
Mean pass@1 & Average over eight runs & \SI{67.7}{\percent} \\
...
Oracle pass@8 & Any run resolves & 408 / 500 = \SI{81.6}{\percent} \\
Cross-agent selector & Official-evaluator result & 374 / 500 = \SI{74.8}{\percent} \\
Selector regret & Oracle minus submitted result & 34 / 500 = \SI{6.8}{\percentagepoint} \\
\bottomrule
\end{tabular}
\end{table}
...
result uses the same 27-billion-parameter agent and phase graph, but the Java xcheck@8
...
evaluator (\SI{74.80}{\percent}, Wilson \SI{95}{\percent} CI [\SI{70.82}{\percent}, \SI{78.41}{\percent}]~\cite{wilson1927score}). The
corresponding internal Docker re-grade resolves 376/500 (\SI{75.2}{\percent}); the
...
returns a wider \SI{95}{\percent} CI of [\SI{67.0}{\percent}, \SI{79.8}{\percent}], reflecting the fact
...
repository, \code{django/django}, accounts for \SI{46.2}{\percent} of the Python benchmark
and resolves at \SI{76.6}{\percent}, just above the global mean. The lowest-rate
repository is \code{pylint-dev/pylint} at \SI{30.0}{\percent} ($n{=}10$). The
auxiliary per-year CSV remains available in the artifact; across
2017--2023, year-level resolution stays in the \SIrange{66}{94}{\percent} band, with the
...
resolves 356/500 (\SI{71.2}{\percent}). Under Benjamini--Hochberg FDR control
($q\le0.05$), the configuration significantly outperforms 16 of 17
curated open-weight peers, including a 480-billion-parameter Qwen3-Coder
...
log-odds-ratio effect sizes with exact \SI{95}{\percent} Clopper--Pearson
...
\code{django/django} ($n=231$), Kozuchi agent resolves \SI{76.6}{\percent}, ahead of the
next open-weight peer in the heatmap at \SI{74.0}{\percent}; on \code{sympy/sympy}
($n=75$) it resolves \SI{76.0}{\percent}, compared with \SI{70.7}{\percent} for the strongest
peers; and on \code{pytest-dev/pytest} ($n=19$) it reaches \SI{84.2}{\percent},
above the nearest peers at \SI{78.9}{\percent}. The same pattern appears on several
medium-size repositories: \code{sphinx-doc/sphinx} is tied for best at
\SI{68.2}{\percent}, and \code{pydata/xarray} is near the top at \SI{81.8}{\percent}. The main
exception is \code{pylint-dev/pylint}, where Kozuchi agent resolves only
\SI{30.0}{\percent} ($3/10$) and several peers reach \SI{40.0}{\percent}. We therefore interpret
...
position analysis to Java, where the same 27-billion-parameter model configuration \textbf{ranks first}
...
churn of the produced patch: from \SI{81.7}{\percent} at \SIrange{1}{4}{\loc} to \SI{41.7}{\percent} at
$>$\SI{100}{\loc}. The point-biserial correlation between LOC churn and
the binary resolution outcome is $r=-0.197$
($p=1.1{\times}10^{-5}$), the strongest single feature signal in
the dataset. The mean LOC added per patch is \SI{10.5}{\loc} (median
\SI{3}{\loc}) for resolved instances versus \SI{21.9}{\loc} (median
\SI{8}{\loc}) for unresolved instances.
...
loop. \code{VERIFY\_PATCH} issues a \code{GIVEUP} in \SI{18.2}{\percent} of
trajectories (90/495, \SI{95}{\percent} CI [\SI{15.0}{\percent}, \SI{21.8}{\percent}]); \code{CODE\_FIX}
itself rolls back in \SI{1.0}{\percent} of cases; every other phase rolls back
zero times. Unresolved trajectories fire roughly \SI{28}{\percent} more
...
Of 126 unresolved instances, 115 (\SI{91.3}{\percent}) are
\code{WRONG\_FIX}: the patch applies cleanly under the harness but
does not flip the hidden \code{FAIL\_TO\_PASS} tests.  6 (\SI{4.8}{\percent}) are
\code{REGRESSION}: the patch applies but breaks at least one
\code{PASS\_TO\_PASS} test. 5 (\SI{4.0}{\percent}) are
...
\midrule
\code{WRONG\_FIX}             & 115 & \SI{91.3}{\percent} \\
\code{REGRESSION}             &   6 &  \SI{4.8}{\percent} \\
\code{MISSING\_ARTEFACT}      &   5 &  \SI{4.0}{\percent} \\
\code{PATCH\_DID\_NOT\_APPLY} &   0 &  \SI{0.0}{\percent} \\
\code{EMPTY\_PATCH}           &   0 &  \SI{0.0}{\percent} \\
\midrule
Total                         & 126 & \SI{100.0}{\percent} \\
\bottomrule
\end{tabular}
\end{table}
...
single edit-layer failure (\SI{0.20}{\percent}). Combined with the failure
...
always emits a syntactically acceptable patch, but in \SI{91.3}{\percent} of
...
existence inflates the unresolved bucket by up to 5/500 (\SI{1.0}{\percentagepoint}).
...
334--343 of 500 instances (\SIrange{66.8}{68.6}{\percent}), with mean
338.6/500 = \SI{67.7}{\percent} and population standard deviation 3.12 resolved
instances (\SI{0.62}{\percentagepoint}). The submitted cross-agent selection raises this
average-run baseline to 374/500 = \SI{74.8}{\percent} on the official evaluator
and 376/500 = \SI{75.2}{\percent} on the internal Docker re-grade, a gain of
35.4--37.4 resolved instances (\SIrange{+7.1}{7.5}{\percentagepoint}) over mean pass@1.
...
1.54), yielding 41/128 = \SI{32.03}{\percent} overall and a \SI{32.3}{\percent} conditional
...
On Python, the cross-agent selector resolves 376/500 = \SI{75.2}{\percent} on the internal
Docker re-grade (Table~\ref{tab:xcheck}) and 374/500 = \SI{74.8}{\percent} on the
...
\SI{72.4}{\percent}; the selector therefore contributes +14 absolute (\SI{+2.8}{\percentagepoint})
...
\SI{92}{\percent} of the internal Docker re-grade oracle ceiling ($376/408$). Pairwise Jaccard similarity
...
tokens, 83{,}100 completion tokens, and \SI{0.77}{\hour} of wall-clock; the p95
tail extends to 1{,}099 LLM calls, 16.6M prompt tokens, and
\SI{1.94}{\hour}. Aggregated over the full eight-run candidate generation, the
...
% source: GPU-hour estimate derived from trajectory_stats.csv
%         (runtime_sec_mean = 3414.16 s over the 495 selected
%         trajectories) x 8 legs x TP=2 GPUs; see Table tab:evidence-map
%         for its evidence status (wall-clock-derived estimate).
Because the trajectory bundle records wall-clock but not
GPU-utilisation traces, we can bound rather than measure GPU cost:
mean per-instance wall-clock is \SI{0.95}{\hour} on the two-GPU tensor-parallel
serving configuration, so summing instance wall-clock across the full
eight-run generation gives an upper bound of roughly
$7.5{\times}10^{3}$ GPU-hours ($8 \times 495 \times 0.95\,\mathrm{h}
\times 2$). This is an upper bound because the vLLM servers batch many
concurrent instances on the same GPUs; per-instance wall-clock
double-counts shared occupancy. Accounted GPU-hours would require
cluster-side telemetry we did not collect (Table~\ref{tab:evidence-map}).
...
shows diminishing returns in the long tail: \SI{80}{\percent} of resolved instances
finish within 653 LLM calls per instance and \SI{95}{\percent} within 1{,}069, but
moving from the \SI{90}{\percent} point to the \SI{99}{\percent} point requires increasing the
...
\textbf{Scaling under tighter budgets.}
For organizations without the infrastructure to spend the full TTS@8
envelope, the measured cost structure suggests three policies, none of
which we have validated with controlled runs: (i) a per-instance
call or token budget that stops the long tail (cutting at the \SI{90}{\percent}
Pareto point of 820 calls sacrifices at most the last decile of
resolved instances in this run); (ii) adaptive candidate count, since
234 of 500 instances are resolved by all eight runs and would be
selected correctly from far fewer candidates; and (iii) skipping the
cross-check matrix when deduplicated patches collapse to a single
candidate. Each policy needs its own controlled study before
deployment.
...
prompt/completion tokens, and wall-clock seconds, with GPU-hours given
only as a wall-clock-derived estimate, because the trajectory bundle
does not record GPU-utilisation traces; accounted GPU-hours require
cluster-side telemetry we did not collect. The ablation in
...
editing; those components require separate controlled reruns
(Table~\ref{tab:evidence-map}). The
...
\caption{Python--Java cross-track evidence for the same 27-billion-parameter Kozuchi agent configuration.}
...
The Java run resolves 41/128 instances (\SI{32.03}{\percent}, Wilson \SI{95}{\percent} CI
[\SI{24.57}{\percent}, \SI{40.54}{\percent}]), ranks 4 of 42 overall, and is first among strict
open-weight submissions. Under BH-FDR control ($q\le0.05$), it
significantly outperforms 34 of 41 Java peers, including all
72-billion-parameter
...
per-phase message share is within $\pm$\SI{5}{\percentagepoint} on every phase, rework still
centres on \code{CODE\_FIX}$\leftrightarrow$\code{VERIFY\_PATCH}, and six
of seven effort medians sit within $\pm\SI{20}{\percent}$ of Python parity. Thus the
\SI{43}{\percentagepoint} headline gap is not a compute gap. The Java failure profile is
instead language/harness-specific: \code{no\_fix\_test\_results} (\SI{25}{\percent}),
\code{report\_valid\_not\_leaderboard\_resolved} (\SI{9}{\percent}), and
\code{anomalous\_test\_pattern} (\SI{6}{\percent}) do not appear on Python; edit-layer
reliability is lower (\SI{96.9}{\percent} vs. \SI{99.8}{\percent}); and \code{elastic/logstash}
...
run does not separately ablate scaffold, and selector
contributions.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
(\SI{32.03}{\percent}) and ranks first among strict open-weight submissions, making
...
SWE-bench (Python) and Multi-SWE-bench Java. Using a
locally hosted open-weight model, no fine-tuning,
eight candidate runs, and cross-agent testing, it resolves 374/500
SWE-bench Verified instances (\SI{74.80}{\percent}, Wilson \SI{95}{\percent} CI [\SI{70.82}{\percent},
\SI{78.41}{\percent}]; repository-clustered bootstrap [\SI{67.0}{\percent}, \SI{79.8}{\percent}]) and 41/128
Multi-SWE-bench Java instances (\SI{32.03}{\percent}) under the same agent,
...
among strict open-weight Java, and recovers \SI{80}{\percent} of resolved
...
mainly semantic or selection failures: \SI{91.3}{\percent} of unresolved instances
...
curve for controlled early stopping and adaptive candidate counts
(\S\ref{subsec:rq5}).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Contributions of Each Team}
\label{sec:contributions}

\paragraph{Fujitsu Research, Japan.}
\begin{itemize}
  \item Led the overall design and implementation of Kozuchi Agent.
  \item Conducted evaluations on SWE-bench.
\end{itemize}

\paragraph{Fujitsu Research of America, USA}
\begin{itemize}
  \item Led the writing of this paper.
  \item Designed the Verifier.
  \item Developed and prepared tools for Java.
  \item Conducted evaluations on Multi-SWE-bench Java.
\end{itemize}

\paragraph{Fujitsu Research of Europe, UK}
\begin{itemize}
  \item Developed the Software Engineering Tool Suite.
\end{itemize}

\paragraph{Fujitsu Research \& Development, China.}
\begin{itemize}
  \item Conducted evaluations of test-time selection.
\end{itemize}
...
\url{https://github.com/marscod/kozuchi-agent-artifact} and Kozuchi Agent source code at: \url{https://github.com/FujitsuResearch/kozuchi-mini-swe-agent}.

The complete agent execution trajectories are available in the Zenodo
archival record (DOI: 10.5281/zenodo.19941649), together with the agent
source code, submission folders, logs, predictions, selector bundle,
...
%% \balance splits the reference list evenly across both columns of the
%% last page, as required by the ACM camera-ready check.
\balance
...
substantially by repository. The Wilson \SI{95}{\percent} CI on the headline rate
treats each instance as independent and therefore understates
uncertainty under repository clustering; the cluster-robust
bootstrap reported in \S\ref{subsec:rq1} ([\SI{67.0}{\percent}, \SI{79.8}{\percent}]) widens
...
\SI{29.2}{\percent} on the 120 instances no Qwen-family peer resolves to \SI{97.5}{\percent}
on the 198 instances every Qwen-family peer resolves; from \SI{6.5}{\percent}
on the 62 instances no closed-frontier peer resolves to \SI{94.6}{\percent} on
...
\midrule
RQ1 & Qwen3.5-27B + TTS@8, official evaluator   & Resolved (Wilson \SI{95}{\percent} CI) & 374/500 = \SI{74.80}{\percent} [70.82, 78.41] \\
RQ1 & Qwen3.5-27B + TTS@8, internal Docker      & Resolved & 376/500 = \SI{75.2}{\percent} \\
RQ1 & Cluster bootstrap (repo-level, $B{=}10{,}000$) & \SI{95}{\percent} CI & [\SI{67.0}{\percent}, \SI{79.8}{\percent}] \\
...
RQ3 & Resolution rate at \SIrange{1}{4}{\loc} vs.\ $>$\SI{100}{\loc} & Bucket rates & \SI{81.7}{\percent} / \SI{41.7}{\percent} \\
RQ3 & \code{VERIFY\_PATCH} \code{GIVEUP} rate   & Trajectories & 90 / 495 = \SI{18.2}{\percent} \\
RQ4 & \code{WRONG\_FIX} share of unresolved     & Count, share  & 115 / 126 = \SI{91.3}{\percent} \\
RQ4 & \code{PATCH\_DID\_NOT\_APPLY} + \code{EMPTY\_PATCH} & Count & 0 / 126 \\
RQ4 & Patch apply rate over 495 produced patches & Rate          & 494 / 495 = \SI{99.80}{\percent} \\
...
RQ5 & Self-tests-only selector                   & Resolved          & 346/500 = \SI{69.2}{\percent} \\
...
RQ5 & TTS@8 vs.\ oracle ceiling                 & Coverage      & $374 / 408 \approx \SI{91.7}{\percent}$ official \\
RQ5 & Selector regret vs.\ oracle               & Gap           & 34 / 500 = \SI{6.8}{\percentagepoint} \\
RQ5 & Per-instance LLM calls (median, p95)      & LLM calls     & 490 / 1{,}099 \\
RQ5 & Per-instance prompt tokens (median)       & Tokens        & 6.20M \\
RQ5 & Compute-resolution Pareto (\SI{80}{\percent} / \SI{95}{\percent} of resolved set) & LLM calls & 653 / 1{,}069 \\
RQ6 & Java Verified headline, same agent/backbone & Resolved (Wilson \SI{95}{\percent} CI) & 41/128 = \SI{32.03}{\percent} [24.57, 40.54] \\
...
(\SI{1.0}{\percent}). Across 146{,}814 bash calls, \SI{95.3}{\percent} with a return-code tag
...
model-free trajectory signals could help recover part of the \SI{6.8}{\percentagepoint}
...
helpers.
\item \textbf{Evaluation.} The official 374/500 (\SI{74.80}{\percent}) score
...
selector reaches $376/408 \approx \SI{92}{\percent}$ of the union ceiling.
\end{itemize}
\fi

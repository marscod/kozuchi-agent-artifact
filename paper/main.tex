...
\title{Kozuchi Agent: A Language-Agnostic Open-Weight Agent for Software Repair}

%% Author roster (acmart): strict per-author affiliation grid. Every author
%% gets their own \author and \affiliation with the mandatory
%% \institution/\city/\country fields; no title-page \email. The four-column
%% grid keeps the 27-author front matter compact.
%% Order matches the prior submission's metadata-derived list.
%% Source: experiments/evaluation/verified/20260326_kozuchi-mini-swe-agent_qwen3.5-27b/metadata.yaml
%% Affiliation keys: 1 = Fujitsu Research of America (Santa Clara, CA, USA);
%%   2 = Fujitsu Research (Kawasaki, Japan); 3 = Fujitsu Research of Europe
%%   (Slough, UK); 4 = Fujitsu Research \& Development (Beijing, China).
...
\author{Yang Zhao}
\affiliation{%
  \institution{Fujitsu Research \& Development}
  \city{Beijing}
  \country{China}
}

\author{Michiaki Takizawa}
\affiliation{%
  \institution{Fujitsu Research}
  \city{Kawasaki}
  \country{Japan}
...
%% Short form for running page headers (full 27-author list overflows).
\renewcommand{\shortauthors}{Bahrami et al.}
%% Let acmart typeset the full abstract before CCS Concepts, Keywords, and
%% ACM Reference Format. Those three blocks may spill to page~2; the
%% first-page publishing-rights footer is emitted by \maketitle itself.
...
\label{sec:intro}
%% Ordinary [t] floats cannot reach page~1 under acmart (\@topnum=0).
\begin{figure}[t]
  \centering
...
artifact-level audit (Data Availability Statement), not by mechanism-removal
...
selection bundles in the public artifact (Data Availability Statement).
...
the Data Availability Statement defines the public artifact boundary.
...
\caption{Evidence map: mechanisms and claims by type.}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.72\textwidth]{\figpath{overall-architecture.pdf}}
...
dependencies (Data Availability Statement), so the artifact pins the model identifier and run
...
directory (full workspace path in the Data Availability Statement), and the
